...
### Fixed
- **Startup Crash**: Fixed a critical configuration error in `tauri.conf.json` causing the application to panic on launch.
- **Video Detection**: Resolved a bug where video files were not detected as videos in the UI due to missing file extension data from the backend.
